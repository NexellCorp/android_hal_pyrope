...
static void dump_video( FFMPEG_STREAM_READER *pReader, const char *outFileName )
{
	...
	for( ;; )
	{
		...
		if( ReadStream( pReader, video_stream, streamBuffer, &readSize, &isKey ) != 0 )
		{
			break;
		}
		if( outFd )
...
int dec_main( int argc, char *argv[] )
{
	...
	hDec = NX_VidDecOpen(vpuCodecType, mp4Class);
	...
	dspInfo.left = dspX;
	dspInfo.top = dspY;
	dspInfo.right = dspWidth+dspX;
	dspInfo.bottom = dspHeight+dspY;
	...
	while( 1 )
	{
		if( 0 == bInit )
		{
			if( seqNeedMoreBuffer == 0 )
			{
				...
				vidRet = NX_VidDecInit2( hDec, &seqIn, &seqOut );
			}
			else
			{
		...
		vidRet = NX_VidDecDecodeFrame( hDec, streamBuffer, pos, &decOut );
		pos = 0;
		...
		if( decOut.outImgIdx >= 0  )
		{
			...
			if( outCount != 0 )
			{
				int gap = curTime - prevTime;
				int sleepTime = frameInterval - gap;
