...
#define GRALLOC_NEXELL_PYROPE   1
#endif
...
struct private_handle_t
{
	...
	int     share_fd;
    /* psw0523 add for pyrope yuv420 planar format */
    int     share_fds[3];
#define GRALLOC_PYROPE_YUV420_NUM_FDS 3
#endif
	int     magic;
	int     flags;
	int     size;
...
#define GRALLOC_ARM_UMP_NUM_INTS 0
	...
	struct ion_handle *ion_hnd1;
	struct ion_handle *ion_hnd2;
    // for nexell ion private
    uint    phys;
    uint    phys1;
    uint    phys2;
    // for hwc
...
#define GRALLOC_ARM_DMA_BUF_NUM_INTS 10
#else
