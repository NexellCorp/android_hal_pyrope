...
#if GRALLOC_ARM_DMA_BUF_MODULE
        int ret;
        // psw0523 fix : mmap only 1 plane
        if ( hnd->format != HAL_PIXEL_FORMAT_YV12 && hnd->format != HAL_PIXEL_FORMAT_YV12_444) {
            ...
            cpu_ptr = (unsigned char *)mmap(NULL, hnd->sizes[0], PROT_READ | PROT_WRITE, MAP_SHARED, hnd->share_fds[0], 0);
            if (MAP_FAILED == cpu_ptr) {
                AERR("mmap( 3plane Y mmap failed: share_fd:%d ) error is %s", hnd->share_fds[0], strerror(errno));
                retval = -errno;
                goto cleanup;
            }
            cpu_ptr1 = (unsigned char *)mmap(NULL, hnd->sizes[1], PROT_READ | PROT_WRITE, MAP_SHARED, hnd->share_fds[1], 0);
            if (MAP_FAILED == cpu_ptr1) {
                AERR("mmap( 3plane CB mmap failed: share_fd:%d ) error is %s", hnd->share_fds[1], strerror(errno));
                munmap(cpu_ptr, hnd->sizes[0]);
                retval = -errno;
                goto cleanup;
            }
            cpu_ptr2 = (unsigned char *)mmap(NULL, hnd->sizes[2], PROT_READ | PROT_WRITE, MAP_SHARED, hnd->share_fds[2], 0);
            if (MAP_FAILED == cpu_ptr2) {
                AERR("mmap( 3plane CR mmap failed: share_fd:%d ) error is %s", hnd->share_fds[2], strerror(errno));
                munmap(cpu_ptr, hnd->sizes[0]);
                munmap(cpu_ptr1, hnd->sizes[1]);
                retval = -errno;
                goto cleanup;
            }
            hnd->base  = reinterpret_cast<int>(cpu_ptr);
            hnd->base1 = reinterpret_cast<int>(cpu_ptr1);
            hnd->base2 = reinterpret_cast<int>(cpu_ptr2);
        }
        pthread_mutex_unlock( &s_map_lock );
        return 0;
#endif
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        if (hnd->flags & private_handle_t::PRIV_FLAGS_USES_UMP) {
            ...
            if ( hnd->format != HAL_PIXEL_FORMAT_YV12 && hnd->format != HAL_PIXEL_FORMAT_YV12_444) {
                ...
                void *base = (void *)hnd->base;
                void *base1 = (void *)hnd->base1;
                void *base2 = (void *)hnd->base2;
                munmap(base, hnd->sizes[0]);
